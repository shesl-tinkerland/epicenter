 ...
 * - {@link ../../document/y-keyvalue/y-keyvalue-lww.ts}: Inner CRDT that handles
 *   conflict resolution (unaware of encryption)
 *
 * @module
 */
...
/**
 * Whether a stored entry surfaced its value, plus the reason when it did not.
 * The `unreadable` arm exists only for an encrypted store: a blob whose key
 * version is missing from the keyring (or whose bytes fail authentication)
 * is stored but cannot be decrypted here, so it has to be visible rather than
 * silently skipped. The plaintext inner store dropped this distinction when the
 * trusted-relay change removed encryption (see ADR 0003); it now lives here,
 * scoped to the one store that can actually produce an unreadable entry.
 */
...
/**
 * The result of a single point read: a {@link KvStoredRead} plus `absent` for a
 * key with no stored entry. Exactly one of three mutually exclusive states, so
 ...
 * The wrapper resolves the whole tri-state with one inner read and one decrypt
 * attempt, instead of decrypting once to get the value and again to recover the
 * failure reason.
 */
...
export function createEncryptedYkvLww<T>(
	...
	return {
		...
		/**
		 * Walk every stored entry once, classified. `inner.entries()` yields the
		 * raw stored value per key (blob or passthrough plaintext); `classify`
		 * turns that into the outward `present` or `unreadable`. One decrypt per
		 * entry, the bulk twin of `read()`.
		 */
		*reads(): IterableIterator<[string, KvStoredRead<T>]> {
			for (const { key, val } of inner.entries()) {
				yield [key, classify(key, val)];
			}
		},
