...
import type { ConnectedChildDoc } from '../document/child-doc-worker.js';
import {
	attachGitAutosave,
...
function connectMountChildDoc(
	...
	return (guid: string): ConnectedChildDoc => {
		...
		const yjsLog = attachYjsLog(ydoc, {
			...
			log: createLogger(`${ctx.mount}-worker-log`),
		});
...
export type NodeMountRuntime = {
	 ...
	 * schema, then hands the connector to `attachChildDocWorker`; this is the one
