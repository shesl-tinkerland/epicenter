...
> Done 2026-04-28: added `packages/workspace/src/shared/schema-partial.ts` with `partialOf(schema, { keep })` (returns arktype `Type<Pick & Partial<Omit>>` via the spike's `pick.and(omit.partial())` recipe; one targeted internal cast on the runtime call to satisfy arktype's literal-string variadic overloads, with the brand-preserving inferred type reconstructed via the `PartialOf<S, K>` helper). Added `packages/workspace/src/daemon/table-actions.ts` exposing `buildTableActions(table, tableName)` -> `{ get, getAllValid, set, update, delete, bulkSet }`, with `update.input = partialOf(table.schema, { keep: ['id'] })`. Re-exported both from `@epicenter/workspace`. Pragmatic deviation: `defineMutation` / `defineQuery` are typed against typebox `TSchema`, but `table.schema` (and `partialOf` outputs) are arktype Types. To bridge them, `buildTableActions` wraps each arktype runtime schema in a `Type.Unsafe<HandlerInput>({})` envelope (the runtime value is still the arktype schema; the typebox shell exists only to project the correct `Static<>` into each handler's parameter type). Validation of the input shape is metadata-only at the daemon today; the wire envelope is the only schema actually run against incoming payloads. Tests in `schema-partial.test.ts` (4 pass) and `table-actions.test.ts` (5 pass) cover round-trip CRUD, `update.input` accepting `{ id }` alone and rejecting `{ title }` / `{ id, _v: 99 }`, and a `@ts-expect-error` line for assigning a plain string to a branded id. Workspace and CLI typechecks clean; pre-existing `system.describe` / `describePeer` failures unrelated.
...
> Done 2026-04-28: added `packages/workspace/src/client/remote.ts` (`buildRemoteWorkspace<T>(client, workspaceName)`), `packages/workspace/src/client/remote-not-supported.ts` (`RemoteNotSupported` error variant), and `packages/workspace/src/client/remote-workspace-types.ts` (mapped types: `RemoteWorkspace<T>`, `RemoteTable<TRow>`, `RemoteAction<A>`, `RemoteCallError`). Re-exported the runtime + types from `@epicenter/workspace`. Tables expose only the wire-callable subset (`get`, `getAllValid`, `set`, `update`, `delete`, `bulkSet`); `filter` / `observe` are runtime stubs that throw `RemoteNotSupported` for dynamic-access guards but are absent from the public type. Actions go through a recursive Proxy: each property access accumulates path segments, and the call-site dispatches `client.run({ workspace, actionPath: path.join('.'), input, waitMs: 5000 })`. `then` is masked everywhere on the path so awaiting an intermediate namespace doesn't poison the chain. `RemoteWorkspace<T>`'s actions slot delegates to the existing `RemoteActions<A>` from `shared/actions.ts` (the same wrapper `peer<T>(...)` uses), so cross-peer RPC and unix-socket RPC stay in lockstep type-wise. Pragmatic deviations from the spec sketch: (1) `DaemonClient` does not yet have `.ping()` / `.close()`; `whenReady` resolves immediately and `[Symbol.dispose]` / `[Symbol.asyncDispose]` are no-ops because the current `daemonClient` opens a fresh `fetch` per call (no pooled state to release). Forward-compatible with a pooled transport. (2) The wire's workspace selector is still `name` not `id` (per Phase 2 deviation); `buildRemoteWorkspace`'s parameter is named `workspaceName` to match. 13 smoke tests in `client/remote.test.ts` cover table CRUD dispatch shape, deeply-nested action path joining, `RemoteNotSupported` on filter/observe, peers fan-out, and disposal callability with a stub `DaemonClient`. Workspace + CLI typechecks clean; pre-existing `system.describe` / `describePeer` failures unrelated.
...
> Done 2026-04-28: the spec's "~600 line delete" estimate was based on outdated file sizes (run.ts 451, list.ts 554, peers.ts 117). Earlier phases (most of the script-first CLI collapse work) had already shrunk these files to 268 / 239 / 115 = 622 lines total. Remaining bulk is argv builders + examples (must keep), error rendering for `RunError` / `PeerMiss` / `RpcError` / `ResolveError` / `DaemonError` (must keep, the wire still produces all of these), JSON-input parsing for `@file` / stdin / inline (must keep), and the action tree printer in list.ts (must keep). The handlers themselves were already 5-line shells dispatching through `getDaemon` + `daemon.{run,list,peers}` from `@epicenter/workspace`. Pragmatic deviation from the spec sketch: the CLI keeps using `getDaemon` (a `Result`-returning helper that already does the connect ceremony plus a `MissingConfig` distinction) for all three commands, rather than swapping to `connectDaemon`. `connectDaemon`'s typed proxy is the right shape for vault scripts (Phase 8); for the CLI it would be a strict regression on `run --peer`, since the proxy hides `peerTarget` / `waitMs`. Net delete this phase: 8 lines (inlined `RunInput` construction in run.ts, collapsed `result.error` early-return in list.ts, dropped unused `RunInput` re-export). Final: run.ts 262, list.ts 237, peers.ts 115 = 614 lines. Typecheck clean for `packages/cli` and `packages/workspace`; no new test failures (pre-existing yargs-message mismatch in `cli.test.ts` only).
