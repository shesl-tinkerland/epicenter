...
Execute Phase 0 and Phase 1 of the implementation plan for two new Epicenter apps: **Betcha** (betcha.so, accountability challenges) and **The Ark** (theark.so, social media), both using Postgres schemas in the existing database. The spec is finalized—read it first.
...
| Decision | Choice |
|---|---|
| Database | PlanetScale Postgres (EU region) via Hyperdrive — one DB, schema-per-app |
| Betcha schema | `pgSchema('betcha')` — tables: `challenge`, `participant`, `ledger` |
| Social schema | `pgSchema('social')` — tables: `profile`, `post`, `comment`, `reaction`, `follow`, `community`, `community_member`, `notification` |
...
1. Create `betcha` and `social` schemas manually in Postgres: `CREATE SCHEMA IF NOT EXISTS "betcha"; CREATE SCHEMA IF NOT EXISTS "social";`
2. Update `apps/api/drizzle.config.ts`: change `schema` to `'./src/db/*.ts'`
3. Expand schema import in `apps/api/src/app.ts`
4. Verify `drizzle-kit generate` produces schema-qualified DDL

### Phase 1: Betcha API Layer
1. Create `apps/api/src/db/betcha-schema.ts` — full Drizzle schema is in the spec (challenge, participant, ledger)
2. Generate and run migration
3. Create `apps/api/src/routes/betcha.ts` — Hono routes for challenge CRUD + participant status changes + balance queries + deadline handler
4. Wire routes into `apps/api/src/app.ts`
5. Verify cross-schema FK and JOIN work
...
- Use `pgSchema('betcha')` — not `pgTable()` — for all Betcha tables
...
- Keep final MVP Betcha schema at exactly 3 tables: `challenge`, `participant`, `ledger`
- Leave `user_payment_method` deferred to Phase 2
- Run `drizzle-kit generate` (not `push`) for non-public schemas — `push` has known bugs (#4969)
...
- Do not use `pgSchema('public')` — it throws. Use `pgTable()` for public schema.
- Do not add cross-app FKs between `betcha` and `social` schemas
