...
Add two new server-authoritative apps to Epicenter—**Betcha** (betcha.so), a Splitwise-inspired accountability/wager app, and **The Ark** (theark.so), a social media platform—sharing a single PlanetScale Postgres database (EU region) with schema-per-app isolation via Cloudflare Hyperdrive. Both reuse Better Auth for identity and the existing OAuth provider infrastructure, but store domain data in dedicated Postgres schemas rather than Yjs/workspace CRDTs.
...
Server-authoritative apps live alongside local-first apps in the same monorepo, sharing auth and UI components but using Postgres schemas + TanStack Query instead of workspace CRDTs. The data model supports cross-app queries via schema-qualified JOINs.
...
| **Shared Postgres, schema-per-app** | Cross-app JOINs, one Hyperdrive, Drizzle `pgSchema()` support, auth FK references | Shared blast radius (mitigated by query timeouts), Drizzle `CREATE SCHEMA` bug (#5274) | ✅ Best fit |

**Key finding**: Drizzle ORM's `pgSchema()` API is well-supported. Cross-schema JOINs work via auto-qualified SQL (`"schema"."table"`). The main bug is `CREATE SCHEMA` without `IF NOT EXISTS` (#5274)—workaround is manual schema creation before running migrations. Other known issues (#4969 sequence alterations, #5609 enum collisions) are avoidable by using `generate` + `migrate` instead of `push` for non-public schemas.

### Drizzle Multi-Schema Patterns
...
Tables in the default `public` schema use `pgTable()` (unchanged). Tables in custom schemas use `pgSchema().table()`:

```typescript
// Existing auth tables — UNCHANGED, stays in public
import { pgTable } from 'drizzle-orm/pg-core';
export const user = pgTable('user', { ... });

// New app tables — explicit schema
import { pgSchema } from 'drizzle-orm/pg-core';
const betcha = pgSchema('betcha');
export const challenge = betcha.table('challenge', { ... });
```

**Critical rule**: `pgSchema('public')` throws an error. Use plain `pgTable()` for public schema tables.

Cross-schema FKs work natively within one Postgres database:

```typescript
// betcha-schema.ts
export const challenge = betcha.table('challenge', {
  createdBy: text('created_by').references(() => user.id, { onDelete: 'set null' }),
// ↑ FK from betcha.challenge to public.user — works because same database
});
```

Cross-schema JOINs work because Drizzle auto-qualifies table names:

```typescript
// Generates: SELECT ... FROM "betcha"."challenge" INNER JOIN "public"."user" ...
...
| Data isolation | Postgres schemas (`betcha`, `social`) | Cross-app JOINs, shared auth FKs, Drizzle `pgSchema()` support |
| Auth tables | Stay in `public` schema (unchanged) | `pgSchema('public')` throws in Drizzle—`pgTable()` is the intended API |
...
| Cross-app FKs | App schemas FK to `public.user` only (`onDelete: 'set null'`) | Preserves settled challenge history when user deletes account. No cross-app FKs. |
...
│  │  public.* betcha.* social.* │ (Yjs sync relay)    │       │
│  └──────────────────────────────────────────────────┘       │
│                                                              │
│  ══════════════ API BOUNDARY ════════════════                │
│                                                              │
...
│
├── public (default schema — UNCHANGED)
│   ├── user                    ← Better Auth
│   ├── session                 ← Better Auth
│   ├── account                 ← Better Auth
...
│   ├── oauth_client            ← OAuth provider (third-party apps register here)
│   ├── oauth_refresh_token     ← OAuth provider
│   ├── oauth_access_token      ← OAuth provider
│   ├── oauth_consent           ← OAuth provider
│   ├── durable_object_instance ← Platform (sync tracking)
│   └── asset                   ← Platform (R2 metadata)
│
├── betcha (new schema)
...
```
betcha.challenge ─────────────FK──▶ public.user (set null — preserves history)
betcha.participant ───────────FK──▶ public.user (cascade) + betcha.challenge (cascade)
betcha.ledger ──────────FK──▶ public.user (from/to set null, actor set null) + betcha.challenge (set null)

social.profile ────────────FK──▶ public.user
social.post ───────────────FK──▶ public.user
social.follow ─────────────FK──▶ public.user (follower + following)
social.comment ────────────FK──▶ public.user + social.post

betcha ←✕→ social    (NO cross-app FKs — linked only via public.user)

### Betcha Drizzle Schema

```typescript
// apps/api/src/db/betcha-schema.ts
import { index, numeric, pgSchema, text, timestamp, unique } from 'drizzle-orm/pg-core';
...
export const betcha = pgSchema('betcha');
 ...
 */
export const participant = betcha.table('participant', {
  id: text('id').primaryKey().$defaultFn(generateId),
 ...
 */
export const ledger = betcha.table('ledger', {
  id: text('id').primaryKey().$defaultFn(generateId),
...
import * as socialSchema from './db/social-schema';

const schema = { ...publicSchema, ...betchaSchema, ...socialSchema };
...
│   │   │   ├── betcha-schema.ts     ← NEW: pgSchema('betcha') tables
│   │   │   └── social-schema.ts     ← NEW: pgSchema('social') tables
│   │   ├── routes/
│   │   │   ├── betcha.ts            ← NEW: challenge CRUD + state machine
│   │   │   └── social.ts            ← NEW: social CRUD + feed
...
apps/api (Cloudflare Worker)
    │
    │ Hono routes → Drizzle ORM (schema-qualified queries)
...
- [ ] **0.2** Create `betcha` and `social` schemas manually in Postgres (`CREATE SCHEMA IF NOT EXISTS "betcha"; CREATE SCHEMA IF NOT EXISTS "social";`) — workaround for Drizzle #5274
- [ ] **0.3** Update `apps/api/drizzle.config.ts`: change `schema` to `'./src/db/*.ts'`
- [ ] **0.4** Verify `drizzle-kit generate` produces schema-qualified DDL (`CREATE TABLE "betcha"."challenge"`)
- [ ] **0.5** Expand schema import in `apps/api/src/app.ts` to include all schema files

### Phase 1: Betcha — API Layer

- [ ] **1.1** Create `apps/api/src/db/betcha-schema.ts` with `pgSchema('betcha')` tables (challenge, participant, ledger)
- [ ] **1.2** Generate and run Drizzle migration for betcha schema
- [ ] **1.3** Create `apps/api/src/routes/betcha.ts` — Hono routes:
  ...
  - Deadline: cron/scheduled handler to auto-expire pending participants
  - Balances: get running balance per friend pair
  - Payment recording only for MVP (`user_payment_method` deferred to Phase 2)
- [ ] **1.4** Auth middleware: validate Bearer token, extract user ID, attach to context
- [ ] **1.5** Wire betcha routes into `apps/api/src/app.ts`
- [ ] **1.6** Verify cross-schema FK and JOIN work in practice
...
- [ ] **4.1** Create `apps/api/src/db/social-schema.ts` with `pgSchema('social')` tables
- [ ] **4.2** Generate and run migration for social schema
- [ ] **4.3** Create `apps/api/src/routes/social.ts` — Hono routes: profiles, posts, feed, follows, communities
...
- [ ] **6.1** API endpoint: "friends with active challenges" (cross-schema JOIN: social.follow + betcha.challenge)
...
5. **Drizzle `push` vs `generate` + `migrate`**: Use `generate` + `migrate` for non-public schemas to avoid known `push` bugs with sequences (#4969).

## Success Criteria

- [ ] `betcha` schema exists in Postgres with exactly 3 tables (`challenge`, `participant`, `ledger`) and FK to `public.user`
- [ ] `social` schema exists in Postgres with tables and FK to `public.user`
...
- [ ] Cross-schema JOIN works (e.g., "followers with active challenges")
- [ ] Both apps authenticate via existing Better Auth
- [ ] Drizzle migrations generate and apply correctly for multi-schema
- [ ] No regressions to existing auth, OAuth provider, or local-first apps
