...
1. **The role split** (ADR-0024): an app-blind **anchor**/**relay** for custody and
   ...
   per-app actors.
2. **Doc-as-wire driven by an observing actor** (ADR-0025): the agent turn is a
...
actor (app-aware, ADR-0024/0025), beside the anchor
...
No server-to-client SSE exists in this picture. The actor appends each token to the
`Y.Text` and Yjs sync carries it to every replica; the UI is a doc observer. The
only token stream is model-to-actor, and with local inference that is in-process.
...
These are not a new taxonomy. They are the order in which the actor + doc-as-wire
layer lights up the already-decided lanes.

```txt
V0  read-only Q&A            Model 1, query tools only. No mutation, no approval,
                             no sandbox. Actor streams prose into the transcript.
...
V2  authored workflows       Model 2 exactly: arbitrary TypeScript, full trust,
                             ...
                             actor. New pieces vs the ai-workflows doc: drive it from
                             any device over doc-as-wire, and wrap any harness behind
                             the actor adapter. Needs OS isolation; deferred + opt-in.
```

V0 and V1 need no new infrastructure beyond the actor (the daemon body plus an
...
```txt
TOOLS THE LOCAL BOOKS ACTOR EXPOSES
...
READ (auto-approve; cannot mutate)
  books_sql_query({ sql })     read-only SQL on the LOCAL mirror, bounded results.
                               ...
                               pipe; the actor runs it locally and returns bounded
                               rows or prose.
...
appends a turn and (optionally) nudges your desktop; the desktop actor runs the model
...
PROVE (hosted sync, no Iroh, extend Zhongwen)
  ...
  5. Real provider/local inference behind startStream(); now the hosted AI route is
     unnecessary for Zhongwen-on-actor.                                            [V0]
...
- **Harness**: pi (`earendil-works/pi`, TypeScript, RPC mode, `tool_call` approval
  ...
  the same actor adapter. The adapter maps harness events to the transcript and the
...
- Chat as the actor's only interface: synthesizing a human-addressed turn (a fake
  ...
  are one durable interface to an actor, not its only one (ADR-0024).
