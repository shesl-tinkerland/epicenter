# 0043. The vault's encryption key comes from the passphrase, not the server; that key source is the only zero-knowledge lever
...
- Losing the passphrase loses the values: there is no server-side recovery, by
  design. See ADR 0042 for the local `forget` path that makes that reachable.
