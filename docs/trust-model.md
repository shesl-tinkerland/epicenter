...
The fix is a local credential source, not a removed login gate. A solo self-host box now mints a single-user bearer token at first boot, persists it `0600`, and prints it once for the operator to paste into the client: a real authenticated user with no third-party issuer behind it, so a homelab needs no Google app to reach its own data (`docs/adr/0072-the-self-host-star-mints-and-persists-its-own-first-boot-bearer.md`). The box runs this solo path or the shared-wiki OAuth path by an explicit `EPICENTER_MODE` launch choice (`solo`, the default, or `shared`), never by sniffing which secrets are set: because the mode IS the data partition, deriving it from mutable credentials would let a secret change silently re-partition a populated box, so the configured OAuth providers must instead agree with the declared mode, checked loudly at boot. Multi-user homelabs that want browser sign-in without a pasted token reach the same gate through a reverse-proxy header in front of their own identity provider, or through better-auth's API keys; those escape hatches are still ahead. The gate never goes away; only where the credential comes from changes (`docs/adr/0070-self-host-adds-no-new-ownership-or-auth-mode.md`).
