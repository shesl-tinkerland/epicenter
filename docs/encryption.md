...
Daemon-side mounts all receive one `MountContext`: `{ epicenterRoot, mount, session }`. There is no local-vs-collaborative kind. `session` is the auth-derived `MountSession` (`{ ownerId, keyring: () => Keyring, openWebSocket, onReconnectSignal, fetch }`) when machine auth is signed in, and `null` when signed out, so the logged-out case is in front of the author. The host's `keyring` closure throws when auth is signed-out, so a late sign-out becomes a thrown error at the next encrypted write or registration site rather than silent ciphertext loss. A mount that needs encryption or the peer plane declares with `defineSessionMount`, which hands the body a non-null `session` and reports `inactive("Sign in to enable <name>.")` when signed out; `attachMountInfrastructure(ydoc, ctx, { ... })` reads `ctx.session` for cloud sync and derives the deterministic `clientID` (from `epicenterRoot`) and the `<mount>-daemon` device id.
...
- `attachLocalStorage(ydoc, { server, ownerId, keyring })` opens `(server, ownerId)`-scoped encrypted IndexedDB persistence and pairs it with a matching `BroadcastChannel` under the same name. The `server` and `ownerId` are snapshotted at attach time (stable for the attachment lifetime), and `keyring` is snapshotted at attach time too: the callback form only guarantees the read happens inside the attach, not earlier. Rotated keys are picked up by the next attach, after which compaction rewrites old-version rows once the update log crosses its trim threshold.
