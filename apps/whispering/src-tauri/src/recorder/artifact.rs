...
fn validate_recording_id(id: &str) -> Result<(), RecorderError> {
    if id.is_empty() {
        return Err(RecorderError::failed("recording id is empty"));
    }
    if id.len() > 128 {
        return Err(RecorderError::failed(format!(
            "recording id is too long ({} chars)",
            id.len()
        )));
    }
    if id.contains('/')
...
pub(crate) fn recordings_dir(app: &AppHandle) -> Result<PathBuf, RecorderError> {
    let app_data = app
        .path()
        .app_data_dir()
        .map_err(|e| RecorderError::failed(format!("resolve app data dir: {e}")))?;
...
fn recording_path(app: &AppHandle, id: &str) -> Result<PathBuf, RecorderError> {
...
fn find_recording_path(app: &AppHandle, id: &str) -> Result<PathBuf, RecorderError> {
    ...
    let entries = std::fs::read_dir(&dir)
        .map_err(|e| RecorderError::failed(format!("read recordings dir {}: {e}", dir.display())))?;
    ...
    Err(RecorderError::failed(format!(
        "no recording artifact found for id '{id}'"
    )))
}
...
fn remove_file_if_present(path: &Path) -> Result<bool, RecorderError> {
...
fn delete_recording_artifacts_matching(
    ...
    for entry in entries {
        ...
        let file_type = entry
            .file_type()
            .map_err(|e| RecorderError::failed(format!("stat artifact {}: {e}", path.display())))?;
...
pub fn write_artifact(
    ...
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent).map_err(|e| {
            RecorderError::failed(format!("create recordings dir {}: {e}", parent.display()))
        })?;
    }
...
pub fn read_artifact_samples(app: &AppHandle, id: &str) -> Result<Vec<f32>, RecorderError> {
...
pub(super) fn delete_artifacts(app: &AppHandle, ids: &[String]) -> Result<u32, RecorderError> {
...
pub(super) fn clear_artifacts(app: &AppHandle) -> Result<u32, RecorderError> {
...
fn write_pcm_as_wav(path: &Path, samples: &[f32]) -> Result<(), RecorderError> {
    ...
    let file = w
        .into_inner()
        .map_err(|e| RecorderError::failed(format!("flush wav {}: {e}", path.display())))?;
    file.sync_all()
        .map_err(|e| RecorderError::failed(format!("sync wav {}: {e}", path.display())))?;
    Ok(())
}

fn io_err(path: &Path) -> impl Fn(std::io::Error) -> RecorderError + '_ {
    move |e: std::io::Error| RecorderError::failed(format!("write wav {}: {e}", path.display()))
}
